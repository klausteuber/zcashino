...
## Win Celebration Architecture (2026-02-20)

### Decision
Inline SVG particles in React components + CSS-only animations for win celebrations. Brand-specific SVG assets generated by Gemini 3.1 Pro.

### Why Not a Library (react-confetti, canvas-confetti)
- Zero new dependencies — no bundle weight, no version management
- Full brand control — libraries don't support dual-brand SVG shapes
- Trivial to maintain — just CSS keyframes and SVG markup

### Why Not Canvas
- Confetti is a decorative overlay, not interactive
- CSS animations with `transform` + `opacity` are GPU-composited and require zero JS per frame
- Canvas would need a render loop and manual lifecycle management (start/stop when overlay mounts/unmounts)

### Why Inline SVGs vs Sprite Sheet
- Only 8 shapes per brand (small, ~4KB total)
- Components mount/unmount with overlay lifecycle (3-3.5s, short-lived)
- No benefit from caching a sprite for elements that exist <4 seconds

### Implementation
- **Components:** `WinCelebration.tsx` exports `ConfettiBurst` (24 SVG particles) and `ChipSlide` (4 SVG chips)
- **Brand detection:** `useEffect` reads `document.body.dataset.brand` set by server layout. One render with default brand before switch — invisible for decorative particles.
- **CSS:** 6 trajectory keyframes, nth-child distribution for stagger. `transform` + `opacity` only (GPU-composited).
- **Mobile:** `@media (max-width: 640px)` hides particles 13-24 and chips 3-4
- **Accessibility:** `aria-hidden="true"` on containers. `prefers-reduced-motion` already suppresses all animations globally.
- **No new state:** Components mount/unmount with existing `resultAnimation` lifecycle (3-3.5s timeout). Particle animation ~1.6s, chip animation ~1.8s — both finish before overlay fades.

### Key Technical Choices
1. **Gemini 3.1 Pro for asset generation.** Separate prompts per brand with exact hex codes, viewBox constraints, and style vocabulary. Zero manual cleanup needed.
2. **8 shapes × 3 repeats = 24 particles.** Enough visual variety without excessive DOM nodes. Mobile drops to 12.
3. **Staggered delays via `animation-delay` + nth-child.** Creates the burst effect without any JS timing logic.
4. **Table flash via `box-shadow` animation.** Uses `inset` box-shadow so it doesn't affect layout. Gold for CypherJester, cyan for 21z (brand override via `body[data-brand="21z"]` selector).
