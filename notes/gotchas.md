...
### Casino SEO and guide styling (2026-09-05)

- Sitemap dates previously used the request time, falsely implying all content had changed. For CypherJester and 21z, omit `lastModified` until a reliable per-page update date exists.
- Shared game copy implied immediate verification. Session-mode hands require seed rotation/reveal; distinguish this from legacy reveal timing and mock demo commitments.
- Tailwind `@theme` aliases can resolve brand variables at the root and retain Cypher colors on 21z. New guide components reference semantic CSS variables directly at the element (for example `text-[var(--accent-primary)]`). Check both hosts visually before changing shared theme definitions.
- Keep guide lookup, metadata, and sitemap selection behind the same brand filter so one casino cannot publish the other casino’s guides.
...
- **Production packaging:** Turbopack traced Cap's package manifest and alias but omitted its `src/*.js` runtime. General casino smoke tests remained green while poker routes could not load. Explicitly trace the package runtime and import it from the built standalone artifact during validation; exercise the poker access route from the standalone server too. Files: `next.config.ts`, `scripts/validate-standalone-artifact.mjs`.

## 2026-09-06 — Deposit setup fails and loops into rate limiting

- Symptom: “Fund with ZEC”/Deposit ends with “Too many attempts” on the shared onboarding dialog.
- Root cause: Production Zallet rejected `z_getnewaccount` with RPC -20 because the migrated Legacy account uses ZIP-32 index 2147483647, above ordinary indices 0–86. Failed session responses omitted the signed cookie, and the modal retriggered setup whenever the parent session ID changed.
- Fix: Only for that exact Zallet error and a verified single-seed Legacy layout, allocate the next ordinary index with the supported `z_recoveraccounts` RPC at the current synced height. Accept only a new UUID and verify its index/seed; conflicts or ambiguous responses fail closed. Never delete or renumber the Legacy account, reuse an existing account, or fall back to shared wallet balances. Preserve signed session ownership after wallet failure; initialize funding once per modal opening and guard concurrent calls. Poll only while an open dialog has a deposit address.
