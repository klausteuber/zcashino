...
import { createSecurityChallenge, validateSecurityProof } from './security-check'
...
export async function issueSecurityChallenge(db: PrismaClient, sessionId: string, nonce: string, request: NextRequest) {
  const status = await accessStatus(db, sessionId)
  if (!status.setupComplete || status.restricted) throw new PokerError('Finish your poker identity setup before checking entry.', 403)
  if (status.nonce !== nonce) throw new PokerError('Security check changed. Refresh and try again.', 409)
  const marker = browserMarker(request)
  if (request.cookies.get(POKER_BROWSER_COOKIE)?.value !== marker) throw new PokerError('Enable the poker browser cookie and reload the security check.', 403)
  if (humanProvider().provider !== 'self-hosted') throw new PokerError('Security verification is not configured.', 503)
  return createSecurityChallenge(status.identityId, nonce, marker, request)
}
export async function validateHumanToken(token: string, nonce: string, request: NextRequest, identityId = '') {
  ...
  if (provider.provider !== 'self-hosted') throw new PokerError('Security verification is not configured. Taking new seats is paused.', 503)
  await validateSecurityProof(token, identityId, nonce, request.cookies.get(POKER_BROWSER_COOKIE)?.value || '', request)
}
export async function verifyHuman(db: PrismaClient, sessionId: string, token: string, nonce: string, request: NextRequest) {
  ...
  if (status.nonce !== nonce) throw new PokerError('Security check already used. Refresh and try again.', 409)
  const marker = browserMarker(request)
  if (request.cookies.get(POKER_BROWSER_COOKIE)?.value !== marker) throw new PokerError('Enable the poker browser cookie and reload the security check.', 403)
  const digest = createHash('sha256').update(token).digest('hex')
  if (await db.pokerHumanToken.findUnique({ where: { digest } })) throw new PokerError('Security check already used.', 409)
  await validateHumanToken(token, nonce, request, status.identityId)
  const now = Date.now()
  await db.$transaction(async tx => {
    ...
    if (result.count !== 1) throw new PokerError('Security check already used. Refresh and try again.', 409)
